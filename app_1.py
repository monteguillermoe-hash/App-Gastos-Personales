...
def populate_filters(data):
    try:
        ...
        if not df.empty:
            # Después del round-trip JSON las fechas vienen como ISO strings ("2026-01-02T00:00:00.000")
            # No usar format="%d/%m/%Y" aquí — pandas lo infiere solo desde ISO
...
def update_dashboard(data, start_date, end_date, rubro, subrubro, medio):
    try:
        ...
        # Después del round-trip JSON las fechas vienen como ISO strings ("2026-01-02T00:00:00.000")
        # pandas las infiere correctamente sin format explícito
        ...
        # Aplicar filtros de rubro/subrubro/medio — dropdowns multi=True entregan lista
        if rubro:
            rubros_sel = rubro if isinstance(rubro, list) else [rubro]
            df = df[df["Rubro Principal"].isin(rubros_sel)]
        if subrubro:
            subrubros_sel = subrubro if isinstance(subrubro, list) else [subrubro]
            df = df[df["Sub-rubro"].isin(subrubros_sel)]
        if medio:
            medios_sel = medio if isinstance(medio, list) else [medio]
            df = df[df["Medio de Pago"].isin(medios_sel)]
